...
function loadStatsPage(){
	...
	var wins = 0;
	var losses = 0;
	for(var i = 0; i<table.rows.length; i++){//row loop
		for(var j = 0; j<table.rows[i].cells.length; j++){//col loop
			if (j==2){
				homeWon = (table.rows[i].cells[j].innerHTML > table.rows[i].cells[j+1].innerHTML)
			}
			if (j==4){
				if (homeWon == true){
					table.rows[i].cells[j].innerHTML = "CU Boulder";
					wins++;
				} else{
					table.rows[i].cells[j].innerHTML = table.rows[i].cells[1].innerHTML;
					losses++;
				}
			}
		}
...
function loadPlayersPage(){
	...
	for (var i = 0; i < players.length; i++){
		var a = "<a href = '#' class = 'dropdown-item' onclick = switchPlayers("+i+")> "+players[i].name + "</a><br>"
		playerSel.innerHTML += a;
  }
}
